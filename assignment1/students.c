 ...
 *
 * Name: Aditya Kothari
 * Email: kotharad@oregonstate.edu
 */
...
void init_student(struct student* student, char* name, int id, float gpa) {
	int len = strlen(name);
	student->name = malloc((len+1)*sizeof(char));
...
struct student* deep_copy_student(struct student* student) {
	int len;
	struct student *coppy_student = malloc(sizeof(struct student));
	coppy_student->id = student->id;
	coppy_student->gpa = student->gpa;
	coppy_student->name = malloc((len+1)*sizeof(char));
	strcpy(coppy_student->name, student->name);
 
  return coppy_student;
}
...
struct student* create_student_array(int num_students, char** names, int* ids,
    float* gpas) {
	struct student *s = malloc(num_students*sizeof(struct student));
	for(int i = 0; i<num_students; i++)
	{
		init_student(&s[i],names[i],ids[i],gpas[i]);//* = [0]
	}
...
//	free(students.name);//fress for the whole thing, just need to do it once	
	free(students);
}
